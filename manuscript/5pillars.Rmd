...
The ramifications of unreliable research include misleading the community, wasting research funds,
slowing scientific progress, eroding public confidence in science and tarnishing the reputation
of associated institutions and colleagues.
...
After identifying some unusual features of the patient group, Baggerly and Coombes attempted
...
Firstly, the labeling of patients in the test set as "responders" or
"non-responders" had been reversed in the original.
Secondly, some of the patients were included more than once (some up to four times) in the analysis,
which is likely to lead to major distortions in results.
...
Additional errors include two cases of reported results for the wrong drug.
Baggerly and Coombes highlight that such mistakes can inadvertently occur when using spreadsheets, and
they note that poor documentation and lack of code contributed to a lack of transparency and poor data
hygiene.
The article underwente two corrigenda, but was untimately retracted in 2011, as the authors were not
...
the flawed data analysis may have had damaging consequences to patients given the wrong drug in the
period 2007-2010, before Duke terminated the trials and suspended the lead author, Dr Anil Potti, who
later resigned.
...
In order for bioinformatic-driven findings to be tranalated to the clinic, they need to be 

as demonstrated by an earlier case study @Baggerly2010-nr. <more detail here>

These consequences highlight the urgency for which we must improve stadards for computational research
...
@Gruning2018-sg; @Akalin2018-eg; @Brito2020-ld; @Peikert2021-eq;
@Rodrigues2023-fw; @DuPre2022-bk].
Inspired by these principles, this article presents the
...
text( 5 , 0.2 , lab="End-to-end automated process")
...
For a bioinformatics project to be reproducible, it needs to be enshrined by code and include all
instructions, from inspecting the raw data to generating the outputs that form the conclusions of the
study.
This automated process removes the need for any manual steps, which are time-consuming and
prone to errors.
...
which would be difficult for graphical tools like spreadsheets or webtools.
Indeed, spreadsheets are prone to data entry, manipulation and formula errors @Panko1996-ko, and are
somewhat overused and misused in the life sciences, leading to problems like inadvertent conversion
of gene names to dates in databases and supplementary Excel files [@Zeeberg2004-js;
@Ziemann2016-pv; @Abeysooriya2021-uz].
...
To facilitate an end-to-end automated process, the code and data need to be "linked", which means the
...
`snakemake` [@Koster2012-sq; @Molder2021-mp], `targets` [@Landau2021] or
...
In this work, authors provided along the traditional research paper a
...
The MyST-NB (short for Markedly Structured Text notebook) is built on top of Jupyter notebooks or Markdown documents
...
practices.
Below we outline some of the benefits.
...
Finally, although DVCSs come from the world of software engineering,
their use has become a standard in computational biology, as witnessed by the numerous guides
and best practices on the subject [@ram2013; @blischak2016; @perez-riverol2016].
...
In the example shown in **Figure 4**, the five containerised applications share the same operating
...
While there are several alternatives such as Podman, Rkt or Apptainer/Singularity, Docker remains the most widely used
containerisation system for web development and research.
Docker now has a large community of users, extensive documentation and
...
```{r,containers,fig.cap="Figure 4. Architecture of containers (left) compared to VMs (right). Adapted from @noauthor_2021-lt.",echo=FALSE,fig.height=6}

library("RColorBrewer")
cols <- brewer.pal(5, "Set2")
cols2 <- adjustcolor( cols, alpha.f = 0.7)
plot(0,type='n',axes=FALSE,ann=FALSE, xlim=c(0, 12), ylim=c(0, 8))
...
polygon(c(6,11.5,11.5,6),c(1,1,1.9,1.9), col=cols2[3], border = NA)

labs3 <- c("Hardware","Hypervisor")
text(c(8.5,8.5),c(0.4,1.4),labels=labs3)

polygon(c(6,7.8,7.8,6),c(2,2,4.5,4.5), col=cols2[2], border = NA)
polygon(c(8,9.6,9.6,8),c(2,2,4.5,4.5), col=cols2[2], border = NA)
polygon(c(9.8,11.5,11.5,9.8),c(2,2,4.5,4.5), col=cols2[2], border = NA)

text(c(6.85,8.8,10.65),c(3.5,3.5,3.5),labels="Guest\nOperating\nSystem")
...
text(8.5,6.7,labels="Virtualized applications")
...
it is nearly impossible to re-identify genomic data @Venkatesaramani2021-tn unless the attackers have
pre-existing genomic data describing an individual or ancestry [@Schadt2012-md; @Gymrek2013-fz].
Therefore, sharing de-identified genomic data and metadata is likely to carry little risk to participant
privacy.
...
more broad sharing of de-identified data.
...
These principles are summarized in **Figure 5**.
...
conforming to FAIR principles (e.g.: GEO, SRA, ENA, PRIDE [@Edgar2002-hs; @Kodama2012-bn; @Deutsch2017-hc]).
However other domains lack well known specialized organized data repositories, such as evolutionary
biology, ecology, cell biology, molecular simulation, social sciences and computing.
...
should be considered. Researchers should be aware, however, that these repositories are often not moderated, 
and that it is their responsibility to make their data FAIR, in particular by providing sufficient metadata.
To this aim, general recommendations have been proposed to enhance data FAIRness in ecology, but which
...
their reproducibility [@Nekrutenko2012; @Garijo2013-zl; @Simoneau2021-kp].
...
[@Ioannidis2009-hv; @Zaringhalam2020-ms; @Samuel2022-ha], so the severity of the problem isn't
well understood.
Part of the problem is the incentive structures in academia, with funding bodies and journals
largely focused on novel research directions rather than confirming previous findings
[@Garijo2013-zl ; @Huang2013-xr; @Kaelin2017-id; @Munafo2017-ty; @Diong2021-is].

(TODO: reviewer 2 point 3: focus on implementation challenges and strategies to overcome.
Expand this section articulating the difficulties with making gold standard reproducibility the norm.
Detail some strategies that can be used.
Case in point: spreadsheet causing gene name errors

1. described in 2004

2. followed by large scale screens in 2016,2022

3. HUGO finally takes action

4. Public awareness

5. Numbers of affected articles diminishes
)

A lack of technical computer skills in many research groups is another culprit.
Specific workshop-based training in these areas could help [@Williams2023-ic].
The [Software Carpentry](https://software-carpentry.org/) initiative is focused on this mission
[@Wilson2014-mh].
However, the most scalable solution would be to provide comprehensive beginner-friendly step-by-step
...
https://bmcresnotes.biomedcentral.com/articles/10.1186/s13104-021-05875-3#Sec2
...
**Mark Ziemann** is a Senior Lecturer in Biotechnology and Bioinformatics at Deakin University.
He has recently been recruited to Burnet Institute as Head of Bioinformatics.
